...
저희 팀은 Vue2기반의 Nuxt.js와 Php기반으로 이루어진 웹 애플리케이션을 운영하고 있습니다. 애플리케이션은 수년간 개발되어 왔으며, 다양한 기능과 복잡한 비즈니스 로직을 포함하고 있습니다. 그러나 프론트엔드 생태계의 빠른 변화로 인해 React로의 점진적인 마이그레이션을 결정했습니다.
...
특히 문제가 된 것은 다이얼로그/Dialog 컴포넌트였습니다. 저희는 새로운 Dialog UI가 필요했지만, 이를 Vue로 개발하게 되면 향후 마이그레이션 과정에서 React로 다시 작성해야 하는 비효율이 발생할 것이었습니다.
...
이러한 한계점에도 불구하고, 저희의 특수한 상황(Dialog UI)에서는 iframe 통합이 가장 적합한 선택이었습니다:
...
```html
<template>
  <div v-show="isVisible" class="dimmed" @click="clickDimmed">
    <iframe
      v-show="isVisible"
      ref="dialogIframe"
      :src="url"
...
export default {
    ...
    isVisible(value) {
      if(value) {
        ...
        this.$refs.dialogIframe.contentWindow.postMessage(
          { type: this.메시지타입.부모_자식_다이얼로그_열기 },
          '*'
        )
        this.preventDocumentScroll()
      } else {
        this.$refs.dialogIframe.contentWindow.postMessage(
          { type: this.메시지타입.부모_자식_다이얼로그_닫기 },
          '*'
        )
        this.dialogHeight = 0
  ...
  methods: {
    // 핵심 메서드: React에서 오는 메시지 처리
    handleChildMessage(event) {
      const { type, payload } = event.data

      switch (type) {
        case this.메시지타입.자식_부모_다이얼로그_열림:
          this.$emit('dialogOpen')
          break

        case this.메시지타입.자식_부모_다이얼로그_닫힘:
          this.$emit('dialogClose')
          break

        case this.메시지타입.자식_부모_다이얼로그_높이_설정:
          this.dialogHeight = payload.height
          this.$refs.dialogIframe.style.height = `${payload.height}px`
          break

        case this.메시지타입.자식_부모_페이지_이동:
          // 페이지 이동 처리 로직
          location.href = this.buildRedirectUrl(payload)
          break
      }
    },

    clickDimmed() {
      // ALERT 타입은 배경 클릭으로 닫히지 않음
      if(this.dialogType === 'ALERT') return
      this.$emit('dialogClose')
    },
...
```typescript
type ExternalDialogProps = {
  dialogHeight?: number
  dialogType?: 'ALERT' | 'CONFIRM'
}

export const useExternalDialog = ({
  dialogHeight = 380,
  dialogType = 'ALERT'
}: ExternalDialogProps) => {
  ...
  useEffect(() => {
    const handleParentMessage = (event: MessageEvent) => {
      ...
      switch (type) {
        case 메시지타입.부모_자식_다이얼로그_열기:
          handleOpen()
          break
        case 메시지타입.부모_자식_다이얼로그_닫기:
          handleClose()
          break
        // 기타 메시지 타입 처리
      }
    ...
    handleUpdateHeight() // 초기 높이 설정

    return () => {
      window.removeEventListener('message', handleParentMessage)
    }
  }, [handleOpen, handleClose, handleUpdateHeight])
...
React Dialog 컴포넌트에서 useExternalDialog 훅을 활용하는 간단한 예:

```tsx
import { useExternalDialog } from './hooks/useExternalDialog'

export const ExternalDialogPage = () => {
...
1. **XSS(Cross-Site Scripting) 공격 방지**
   ...
   - 예: `<iframe sandbox="allow-scripts allow-same-origin"></iframe>`
...
현재 저희 구현에서는 보안을 위해 다음과 같은 개선을 계획하고 있습니다:
...
저희의 목표는 전체 Vue 애플리케이션을 React로 마이그레이션하는 것입니다. 이 과정이 완료되면 iframe 통합 계층은 제거하고 직접적인 React 컴포넌트로 대체할 예정입니다. 이는 성능 향상과 코드 복잡성 감소에 기여할 것입니다.
