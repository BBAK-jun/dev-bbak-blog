...
function Dialog() {
  ...
  useEffect(() => {
    ...
    // 이벤트 리스너 등록
    window.addEventListener('dialog:open', onOpen);
    window.addEventListener('dialog:close', onClose);

    return () => {
      window.removeEventListener('dialog:open', onOpen);
      window.removeEventListener('dialog:close', onClose);
    };
  }, []);
