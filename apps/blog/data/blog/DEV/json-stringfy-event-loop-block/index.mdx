...
summary: 비동기인데 왜 멈춰? 라고 했던 과거의 나에게 말해주고 싶다.
---
...
이것도 깨달은 게 있다. JavaScript 문자열은 **불변(immutable)** 이다.
...
result = "";
result += "{";                    // 새 문자열 (2 byte)
result += '"id":';                // 새 문자열 (7 byte), 기존 복사
result += "1";                    // 새 문자열 (8 byte), 기존 복사
result += ",";                    // 새 문자열 (9 byte), 기존 복사
result += '"name":"test"';        // 새 문자열 (22 byte), 기존 복사
result += "}";                    // 새 문자열 (23 byte), 기존 복사
```
...
시간 복잡도가 O(n²)에 가까워진다. 20만 개 항목이면? 어마어마한 복사 비용이다.

## V8의 Rope 최적화

V8은 이걸 **Rope** 자료구조로 완화한다.
...
**이것만으로 직렬화 시간이 60% 감소했다.** 500ms → 200ms.
...
처음엔 막막했는데, 하나씩 이해하고 나니 해결책이 보였다.

"Node.js는 비동기니까 알아서 잘 처리하겠지"라는 막연한 믿음이 문제였다. **동기 CPU 작업은 여전히 블로킹을 일으킨다**는 걸 제대로 이해하지 못했던 거다.

이제는 큰 데이터를 다룰 때 항상 생각한다:
- 데이터를 줄일 수 있을까?
- 나눠서 처리할 수 있을까?
- 다른 프로세스/워커로 오프로딩할 수 있을까?

여러분도 비슷한 문제를 겪고 있다면, 이 글이 도움이 되길 바란다.
