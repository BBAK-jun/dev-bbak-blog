...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 400">
    ...
    <line x1="350" y1="200" x2="470" y2="200" stroke="#333333" stroke-width="3" />
...
프론트엔드 개발에서 상태 관리를 자바스크립트의 클로저를 통해 구현하는 경우, 객체지향적 사고를 깊이 적용할 필요는 없을 때가 많습니다.
자바스크립트의 클로저는 간결하게 상태를 유지할 수 있는 방법이지만, 구조화된 데이터와 복잡한 상호작용을 처리할 때는 제한적일 수 있습니다.
사실, 자바스크립트의 클래스 문법은 데이터를 표현하는 데 매우 적합한 구조입니다.
클래스는 객체는 아니지만 객체처럼 사용할 수 있기 때문에 상태(state)와 행동(behavior)을 관리하는 데 강력한 도구가 됩니다.
...
- `예시` : 용자가 작성한 게시글의 내용이 규정된 길이를 초과하지 않는지, 특정 금칙어가 포함되지 않았는지 확인합니다. 이후 데이터를 데이터베이스에 저장하거나, 필요시 알림 기능을 호출하는 등의 작업을 수행합니다.
...
왜 Valtio가 객체지향 프로그래밍과 잘 어울리는지 조금 더 알아보겠습니다.
...
ViewModel의 UI와 상호작용을 처리하며 상태관리를 담당해야합니다. 하지만 ViewModel이 너무 많은 책임을 갖게 되면, 코드가 복잡해지고 유지보수가 어려워질 수 있습니다.

이를 해결하기 위해 Manager 객체를 도입할 수 있습니다. Manager 객체는 ViewModel의 역할을 강화하고, ViewModel의 책임을 분산시킴으로써 코드의 가독성과 유지보수성을 높일 수 있습니다.
