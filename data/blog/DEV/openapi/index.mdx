...
보통 openAPI 또는 OpenAPI Specification(OAS)라고 부르는데, 이는 Restful API를 정의된 규치겡 맞게 API 스펙을 json or yaml 파일로 표현하는 방식을 의미한다.
...
|                   | backend                | frontend              |
| ----------------- | ---------------------- | --------------------- |
| 헬스체크          | fetchLoginUser         | fetchAuth             |
| 로그인            | userLogin              | login                 |
| 로그아웃          | userLogout             | logout                |
| 액세스토큰 검증   | 존재하지않음           | validationAccessToken |
| 액세스토큰 재발급 | restoreUserAccessToken | restoreAccessToken    |
...
| 로그인            | login              | login              |
| 로그아웃          | logout             | logout             |
| 액세스토큰 검증   | healthCheck        | healthCheck        |
| 액세스토큰 재발급 | restoreAccessToken | restoreAccessToken |
...
[[휴톰] 회고글: Medical Data Manager 근영님](https://www.linkedin.com/pulse/%25ED%259C%25B4%25ED%2586%25B0-%25ED%259A%258C%25EA%25B3%25A0%25EA%25B8%2580-medical-data-manager-%25EA%25B7%25BC%25EC%2598%2581%25EB%258B%2598-hutom?trackingId=MIbe%2BJSpeyDYNwV%2Bh4OzKQ%3D%3D/?trackingId=MIbe+JSpeyDYNwV+h4OzKQ==)
